...
function validateListSizeAppliedToList(application, parent, errorCollector) {
    ...
    if (!sizedFields.length && parent.type && !(0, definitions_1.isListType)(parent.type) && !isNonNullListType(parent.type)) {
...
function validateSizedFieldsAreValidLists(application, parent, errorCollector) {
    ...
    if (sizedFields.length && parent.type) {
        const baseParentType = (0, definitions_1.baseType)(parent.type);
        if (!(0, definitions_1.isCompositeType)(baseParentType)) {
            ...
            for (const sizedFieldName of sizedFields) {
                ...
                if (!sizedField) {
                    errorCollector.push(error_1.ERRORS.LIST_SIZE_INVALID_SIZED_FIELD.err(`Sized field "${sizedFieldName}" is not a field on type "${baseParentType.coordinate}"`, { nodes: (0, definitions_1.sourceASTs)(application, parent) }));
                }
...
class FederationMetadata {
    ...
    cacheTagDirective() {
        return this.getPost20FederationDirective(federationSpec_1.FederationDirectiveName.CACHE_TAG);
    }
...
exports.FEDERATION2_LINK_WITH_FULL_IMPORTS = '@link(url: "https://specs.apollo.dev/federation/v2.12", import: ["@key", "@requires", "@provides", "@external", "@tag", "@extends", "@shareable", "@inaccessible", "@override", "@composeDirective", "@interfaceObject", "@authenticated", "@requiresScopes", "@policy", "@context", "@fromContext", "@cost", "@listSize", "@cacheTag"])';
exports.FEDERATION2_LINK_WITH_AUTO_EXPANDED_IMPORTS = '@link(url: "https://specs.apollo.dev/federation/v2.12", import: ["@key", "@requires", "@provides", "@external", "@tag", "@extends", "@shareable", "@inaccessible", "@override", "@composeDirective", "@interfaceObject"])';
...
function validateNoAuthenticationOnInterfaces(metadata, errorCollector) {
    ...
    [authenticatedDirective, requiresScopesDirective, policyDirective].forEach((directive) => {
        for (const application of directive.applications()) {
            ...
            if (((0, definitions_1.isElementNamedType)(element) &&
                ((0, definitions_1.isInterfaceType)(element) || isInterfaceObjectType(element))) ||
                ((0, definitions_1.isFieldDefinition)(element) && (0, definitions_1.isInterfaceType)(element.parent))) {
                let kind = '';
                switch (element.kind) {
                ...
                errorCollector.push(error_1.ERRORS.AUTH_REQUIREMENTS_APPLIED_ON_INTERFACE.err(`Invalid use of @${directive.name} on ${kind} "${element.coordinate}": @${directive.name} cannot be applied on interfaces, interface fields and interface objects`, { nodes: (0, definitions_1.sourceASTs)(application, element.parent) }));
            }
        }
    });
}
