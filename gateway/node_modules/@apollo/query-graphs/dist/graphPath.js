...
function advancePathWithDirectTransition(path, transition, conditionResolver, overrideConditions) {
    ...
    if (transition.kind === 'FieldCollection'
        ...
        (0, federation_internals_1.assert)(!isUnadvanceableClosures(updatedPath), () => `Advancing ${path} for ${transition} gave ${updatedPath}`);
    ...
    for (const edge of path.nextEdges()) {
        ...
        const conditionResolution = canSatisfyConditions(path, edge, conditionResolver, pathContext_1.emptyContext, [], [], getFieldParentTypeForEdge);
        if (conditionResolution.satisfied) {
...
function canSatisfyConditions(path, edge, conditionResolver, context, excludedEdges, excludedConditions, getFieldParentType) {
    ...
    if (requiredContexts.length > 0) {
                ...
                if (e !== null && !contextMap.has(cxt.namedParameter) && !someSelectionUnsatisfied) {
                    const matches = Array.from(cxt.typesWithContextSet).some(t => {
                        if (parentType) {
                            ...
                            if ((0, federation_internals_1.isObjectType)(parentInSupergraph) || (0, federation_internals_1.isInterfaceType)(parentInSupergraph)) {
                                if (parentInSupergraph.interfaces().some(i => i.name === t)) {
                                    return true;
                                }
                            }
                            const tInSupergraph = parentInSupergraph.schema().type(t);
                            if (tInSupergraph && (0, federation_internals_1.isUnionType)(tInSupergraph)) {
                    ...
                    if (parentType && matches) {
                        const parentInSupergraph = path.graph.schema.type(parentType.name);
                        (0, federation_internals_1.assert)((0, federation_internals_1.isCompositeType)(parentInSupergraph), "Parent type should be composite type");
                        let selectionSet = (0, federation_internals_1.parseSelectionSet)({ parentType: parentInSupergraph, source: cxt.selection });
                        selectionSet = selectionSet.lazyMap((selection) => {
                            if (selection.kind === 'FragmentSelection') {
                                if (selection.element.typeCondition && (0, federation_internals_1.isObjectType)(selection.element.typeCondition)) {
                                    if (!(0, federation_internals_1.possibleRuntimeTypes)(parentInSupergraph).includes(selection.element.typeCondition)) {
                                        return undefined;
                                    }
                                }
                            }
                        ...
                        (0, federation_internals_1.assert)(edge.transition.kind === 'FieldCollection', () => `Expected edge to be a FieldCollection edge, got ${edge.transition.kind}`);
                        const argIndices = path.graph.subgraphToArgIndices.get(cxt.subgraphName);
                        (0, federation_internals_1.assert)(argIndices, () => `Expected to find arg indices for subgraph ${cxt.subgraphName}`);
                        const id = argIndices.get(cxt.coordinate);
                        (0, federation_internals_1.assert)(id !== undefined, () => `Expected to find arg index for ${cxt.coordinate}`);
        ...
        if (requiredContexts.some(c => !contextMap.has(c.namedParameter))) {
            ...
            return { ...exports.unsatisfiedConditionsResolution, unsatisfiedConditionReason: UnsatisfiedConditionReason.NO_CONTEXT_SET };
        }
...
class SimultaneousPathsWithLazyIndirectPaths {
    ...
    computeIndirectPaths(idx) {
        return advancePathWithNonCollectingAndTypePreservingTransitions(this.paths[idx], this.context, this.conditionResolver, this.excludedNonCollectingEdges, this.excludedConditionsOnNonCollectingEdges, (_t, context) => context, opPathTriggerToEdge, this.overrideConditions, getFieldParentTypeForOpTrigger);
    }
    toString() {
...
function createInitialOptions(initialPath, initialContext, conditionResolver, excludedEdges, excludedConditions, overrideConditions, initialSubgraphConstraint) {
...
function edgeForField(graph, vertex, field, overrideConditions) {
    const candidates = graph.outEdges(vertex)
        .filter(e => {
        var _a;
        return e.transition.kind === 'FieldCollection'
            && field.selects(e.transition.definition, true, undefined, (_a = e.requiredContexts) === null || _a === void 0 ? void 0 : _a.map(c => c.namedParameter))
...
const getFieldParentTypeForOpTrigger = (trigger) => {
    if (!(0, pathContext_1.isPathContext)(trigger)) {
        if (trigger.kind === 'Field') {
...
const getFieldParentTypeForEdge = (transition) => {
    if (transition.kind === 'FieldCollection') {
        const type = transition.definition.parent;
        if (!type || (0, federation_internals_1.isScalarType)(type) || (0, federation_internals_1.isEnumType)(type)) {
            return null;
        }
        if ((0, federation_internals_1.isObjectType)(type) || (0, federation_internals_1.isInterfaceType)(type) || (0, federation_internals_1.isUnionType)(type)) {
            return type;
        }
    }
    return null;
