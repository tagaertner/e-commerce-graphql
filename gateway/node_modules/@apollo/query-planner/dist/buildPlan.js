...
class QueryPlanningTraversal {
    constructor(parameters, selectionSet, startFetchIdGen, hasDefers, rootKind, costFunction, initialContext, typeConditionedFetching, nonLocalSelectionsState, initialSubgraphConstraint, excludedDestinations = [], excludedConditions = []) {
        var _a;
        ...
        const initialOptions = (0, query_graphs_1.createInitialOptions)(initialPath, initialContext, this.conditionResolver, excludedDestinations, excludedConditions, parameters.overrideConditions, initialSubgraphConstraint);
        ...
        if (this.parameters.federatedQueryGraph.nonLocalSelectionsMetadata
            ...
            if (this.parameters.federatedQueryGraph.nonLocalSelectionsMetadata
                .checkNonLocalSelectionsLimitExceededAtRoot(this.stack, nonLocalSelectionsState, this.parameters.supergraphSchema, this.parameters.inconsistentAbstractTypesRuntimes, this.parameters.overrideConditions, initialSubgraphConstraint !== null)) {
    ...
    resolveConditionPlan(edge, context, excludedDestinations, excludedConditions, extraConditions) {
        const bestPlan = new QueryPlanningTraversal({
            ...this.parameters,
            root: edge.head,
        }, extraConditions !== null && extraConditions !== void 0 ? extraConditions : edge.conditions, 0, false, 'query', this.costFunction, context, this.typeConditionedFetching, null, null, excludedDestinations, (0, query_graphs_1.addConditionExclusion)(excludedConditions, edge.conditions)).findBestPlan();
...
function computePlanInternal({ parameters, hasDefers, nonLocalSelectionsState, }) {
    ...
    if (operation.rootKind === 'mutation') {
        const dependencyGraphs = computeRootSerialDependencyGraphForMutation(parameters, hasDefers, nonLocalSelectionsState);
...
function computeRootParallelBestPlan(parameters, selection, startFetchIdGen, hasDefers, nonLocalSelectionsState) {
    const planningTraversal = new QueryPlanningTraversal(parameters, selection, startFetchIdGen, hasDefers, parameters.root.rootKind, defaultCostFunction, query_graphs_1.emptyContext, parameters.config.typeConditionedFetching, nonLocalSelectionsState, null);
    const plan = planningTraversal.findBestPlan();
    return plan !== null && plan !== void 0 ? plan : createEmptyPlan(parameters);
}
function computeRootParallelBestPlanForMutation(parameters, selection, startFetchIdGen, hasDefers, nonLocalSelectionsState) {
    let bestPlan;
    ...
    for (const mutationSubgraph of mutationSubgraphs) {
        const planningTraversal = new QueryPlanningTraversal(parameters, selection, startFetchIdGen, hasDefers, parameters.root.rootKind, defaultCostFunction, query_graphs_1.emptyContext, parameters.config.typeConditionedFetching, nonLocalSelectionsState, mutationSubgraph);
        const plan = planningTraversal.findBestPlan();
    ...
    if (!bestPlan) {
        throw new Error(`Was not able to plan ${parameters.operation.toString(false, false)} starting from a single subgraph: This shouldn't have happened.`);
    }
    return bestPlan;
}
...
function computeRootSerialDependencyGraphForMutation(parameters, hasDefers, nonLocalSelectionsState) {
    ...
    let [prevDepGraph, prevPaths] = computeRootParallelBestPlanForMutation(parameters, splittedRoots[0], startingFetchId, hasDefers, nonLocalSelectionsState);
    let prevSubgraph = onlyRootSubgraph(prevDepGraph);
    for (let i = 1; i < splittedRoots.length; i++) {
        const [newDepGraph, newPaths] = computeRootParallelBestPlanForMutation(parameters, splittedRoots[i], prevDepGraph.nextFetchId(), hasDefers, nonLocalSelectionsState);
...
function createPostRequiresGroup(dependencyGraph, edge, group, path, context, result) {
    ...
    if (triedToMerge) {
        ...
        const keyCondition = (0, query_graphs_1.getLocallySatisfiableKey)(dependencyGraph.federatedQueryGraph, edge.head);
        (0, federation_internals_1.assert)(keyCondition, () => `Due to @requires, validation should have required a key to be present for ${edge}`);
        ...
        addPostRequireInputs(dependencyGraph, requirePath, entityType, edge, context, preRequireGroup, postRequireGroup);
        return {
