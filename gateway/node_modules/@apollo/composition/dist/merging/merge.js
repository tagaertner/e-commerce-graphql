...
exports.AuthValidator = exports.mergeSubgraphs = exports.isMergeFailure = exports.isMergeSuccessful = exports.sourcesFromArray = void 0;
...
class Merger {
    constructor(subgraphs, options) {
        ...
        this.directivesUsingJoinDirective = new Set();
        this.accessControlDirectivesInSupergraph = [];
    ...
    validateAndMaybeAddSpecs(directivesMergeInfo) {
        ...
        for (const { specInSupergraph, directives } of supergraphInfoByIdentity.values()) {
            ...
            for (const { nameInFeature, nameInSupergraph, compositionSpec } of directives) {
                ...
                if (specInSupergraph.identity === federation_internals_1.AuthenticatedSpecDefinition.identity && nameInFeature === specInSupergraph.url.name) {
                    ...
                    if (authenticatedDirective) {
                        this.accessControlDirectivesInSupergraph.push({
                            name: federation_internals_1.FederationDirectiveName.AUTHENTICATED,
                            nameInSupergraph: authenticatedDirective.name,
                        });
                    }
                }
                if (specInSupergraph.identity === federation_internals_1.RequiresScopesSpecDefinition.identity && nameInFeature === specInSupergraph.url.name) {
                    const requiresScopesDirective = this.merged.directive(nameInSupergraph);
                    if (requiresScopesDirective) {
                        this.accessControlDirectivesInSupergraph.push({
                            name: federation_internals_1.FederationDirectiveName.REQUIRES_SCOPES,
                            nameInSupergraph: requiresScopesDirective.name,
                        });
                    }
                }
                if (specInSupergraph.identity === federation_internals_1.PolicySpecDefinition.identity && nameInFeature === specInSupergraph.url.name) {
                    const policyDirective = this.merged.directive(nameInSupergraph);
                    if (policyDirective) {
                        this.accessControlDirectivesInSupergraph.push({
                            name: federation_internals_1.FederationDirectiveName.POLICY,
    ...
    copyNonJoinAppliedDirectives(source, dest) {
        if (dest instanceof federation_internals_1.FieldDefinition) {
            for (const { name, nameInSupergraph } of this.accessControlDirectivesInSupergraph) {
                ...
                if (additionalSources.length > 0) {
                    this.mergeAppliedDirective(nameInSupergraph, sourcesFromArray(additionalSources), dest);
                }
            }
        }
        source.appliedDirectives.forEach((d) => {
            ...
            if (this.accessControlDirectivesInSupergraph.some(({ nameInSupergraph }) => d.name === nameInSupergraph)) {
                return;
            }
    ...
    recordTypeAppliedDirectivesToMerge(sources, dest) {
        var _a;
        const inaccessibleName = (_a = this.inaccessibleDirectiveInSupergraph) === null || _a === void 0 ? void 0 : _a.name;
        const names = this.gatherAppliedDirectiveNames(sources);
        if (inaccessibleName && names.has(inaccessibleName)) {
        ...
        if (dest instanceof federation_internals_1.InterfaceType) {
            for (const { name, nameInSupergraph } of this.accessControlDirectivesInSupergraph) {
                let additionalSources = this.accessControlAdditionalSources().get(`${dest.coordinate}_${name}`);
                if (!additionalSources) {
                    additionalSources = [];
    ...
    recordFieldAppliedDirectivesToMerge(sources, dest) {
        var _a;
        const inaccessibleName = (_a = this.inaccessibleDirectiveInSupergraph) === null || _a === void 0 ? void 0 : _a.name;
        const names = this.gatherAppliedDirectiveNames(sources);
        if (inaccessibleName && names.has(inaccessibleName)) {
    ...
    accessControlAdditionalSources() {
        if (this.__accessControlAdditionalSources === undefined) {
            const sourceMap = new Map();
            const intfToImplMap = new Map();
            const implToIntfMap = new Map();
            ...
            this.subgraphs.values().forEach((subgraph) => {
                ...
                for (const { name, directive } of accessControlDirectives) {
                    for (const application of directive.applications()) {
                        ...
                        if (candidate instanceof federation_internals_1.ObjectType) {
                        ...
                        if (candidate instanceof federation_internals_1.FieldDefinition && !metadata.isFieldExternal(candidate)) {
                            const parent = candidate.parent;
                            if (parent instanceof federation_internals_1.ObjectType) {
                                const objectName = parent.name;
                                if (metadata.isInterfaceObjectType(parent)) {
                                    const implementations = intfToImplMap.get(objectName);
                                    if (implementations) {
                                        const otherInterfaces = new Set();
    ...
    recordAdditionalSource(sourceMap, key, source) {
    ...
    addJoinDirectiveDirectives(sources, dest) {
        ...
        for (const [idx, source] of sources.entries()) {
            ...
            for (const directive of source.appliedDirectives) {
                const sourceFeature = coreFeaturesInSource === null || coreFeaturesInSource === void 0 ? void 0 : coreFeaturesInSource.sourceFeature(directive);
                let shouldIncludeAsJoinDirective = false;
                let directiveNameForJoinDirective = directive.name;
                if (sourceFeature && sourceFeature.feature.url.identity == federation_internals_1.linkIdentity) {
                    ...
                    shouldIncludeAsJoinDirective = this.shouldUseJoinDirectiveForURL(sourceFeature === null || sourceFeature === void 0 ? void 0 : sourceFeature.feature.url);
                    if (!shouldIncludeAsJoinDirective
                        && this.directivesUsingJoinDirective.has(directive.name)) {
                        shouldIncludeAsJoinDirective = true;
                        if (sourceFeature) {
                            directiveNameForJoinDirective = federation_internals_1.CoreFeature.directiveNameInSchemaForCoreArguments(sourceFeature.feature.url, sourceFeature.feature.url.name, [], sourceFeature.nameInFeature);
                        }
                    }
                }
                if (shouldIncludeAsJoinDirective) {
                    const existingJoins = ((_a = joinsByDirectiveName[directiveNameForJoinDirective]) !== null && _a !== void 0 ? _a : (joinsByDirectiveName[directiveNameForJoinDirective] = []));
                    let found = false;
    ...
    getFieldsWithRequiresDirective() {
        return this.getFieldsWithAppliedDirective((subgraph) => subgraph.metadata().requiresDirective(), (application) => {
            const field = application.parent;
            (0, federation_internals_1.assert)((0, federation_internals_1.isFieldDefinition)(field), () => `Expected ${application.parent} to be a field`);
            return field;
        });
...
class AuthValidator {
    constructor(schema, joinSpec, subgraphNamesToJoinSpecName) {
        var _a, _b, _c, _d, _e;
        this.schema = schema;
        this.joinFieldDirective = joinSpec.fieldDirective(this.schema);
        this.joinSpecNamesToSubgraphNames = new Map(Array.from(subgraphNamesToJoinSpecName, entry => entry.reverse()));
        const authenticatedFeature = (_a = this.schema.coreFeatures) === null || _a === void 0 ? void 0 : _a.getByIdentity(federation_internals_1.AuthenticatedSpecDefinition.identity);
        const authenticatedSpec = authenticatedFeature && federation_internals_1.AUTHENTICATED_VERSIONS.find(authenticatedFeature.url.version);
        this.authenticatedDirective = authenticatedSpec === null || authenticatedSpec === void 0 ? void 0 : authenticatedSpec.authenticatedDirective(this.schema);
        const requiresScopesFeature = (_b = this.schema.coreFeatures) === null || _b === void 0 ? void 0 : _b.getByIdentity(federation_internals_1.RequiresScopesSpecDefinition.identity);
        const requiresScopesSpec = requiresScopesFeature && federation_internals_1.REQUIRES_SCOPES_VERSIONS.find(requiresScopesFeature.url.version);
        this.requiresScopesDirective = requiresScopesSpec === null || requiresScopesSpec === void 0 ? void 0 : requiresScopesSpec.requiresScopesDirective(this.schema);
        const policyFeature = (_c = this.schema.coreFeatures) === null || _c === void 0 ? void 0 : _c.getByIdentity(federation_internals_1.PolicySpecDefinition.identity);
        const policySpec = policyFeature && federation_internals_1.POLICY_VERSIONS.find(policyFeature.url.version);
        this.policyDirective = policySpec === null || policySpec === void 0 ? void 0 : policySpec.policyDirective(this.schema);
        const contextFeature = (_d = this.schema.coreFeatures) === null || _d === void 0 ? void 0 : _d.getByIdentity(federation_internals_1.ContextSpecDefinition.identity);
        const contextSpec = contextFeature && federation_internals_1.CONTEXT_VERSIONS.find(contextFeature.url.version);
        const contextDirective = contextSpec === null || contextSpec === void 0 ? void 0 : contextSpec.contextDirective(this.schema);
        const contextApplications = Array.from((_e = contextDirective === null || contextDirective === void 0 ? void 0 : contextDirective.applications()) !== null && _e !== void 0 ? _e : []);
        this.contexts = new Map();
        contextApplications.forEach((context) => {
            var _a;
            const contextName = context.arguments().name;
            const contextType = context.parent;
            const types = (_a = this.contexts.get(contextName)) !== null && _a !== void 0 ? _a : [];
            types.push(contextType.name);
            this.contexts.set(contextName, types);
        });
    }
    validateRequiresFieldSet(coordinate) {
        const fieldCoordinate = coordinate.split('.');
        (0, federation_internals_1.assert)(fieldCoordinate && fieldCoordinate.length == 2, `Valid coordinate for field "${coordinate}" with @requires`);
        const type = this.schema.type(fieldCoordinate[0]);
        (0, federation_internals_1.assert)(type instanceof federation_internals_1.ObjectType || type instanceof federation_internals_1.InterfaceType, 'Type "${fieldCoordinate[0]}" exists in the schema');
        const field = type.field(fieldCoordinate[1]);
        (0, federation_internals_1.assert)(field instanceof federation_internals_1.FieldDefinition, `Field "${coordinate}" exists in the schema`);
        ...
        for (const joinDirectiveOnRequires of joinDirectivesOnRequires) {
            ...
            if (requiresFieldSet) {
                const requiresSelectionSet = (0, federation_internals_1.parseSelectionSet)({ parentType: type, source: requiresFieldSet });
                try {
                    ...
                    if (subgraph) {
                        errors.push((0, federation_internals_1.addSubgraphToError)(e, subgraph));
                    }
                    else {
                        errors.push(e);
                    }
    ...
    validateFromContext(coordinate) {
        const fieldCoordinate = coordinate.split('.');
        (0, federation_internals_1.assert)(fieldCoordinate && fieldCoordinate.length == 2, `Valid coordinate for field "${coordinate}" with @requires`);
        const type = this.schema.type(fieldCoordinate[0]);
        (0, federation_internals_1.assert)(type instanceof federation_internals_1.ObjectType || type instanceof federation_internals_1.InterfaceType, 'Type "${fieldCoordinate[0]}" exists in the schema');
        ...
        for (const joinDirectiveOnFromContext of joinDirectivesOnFromContext) {
            ...
            if (contexts) {
                for (const context of contexts) {
                    ...
                    (0, federation_internals_1.assert)(targetTypeNames, 'Contexts exists');
                    for (const targetTypeName of targetTypeNames) {
                        const targetType = this.schema.type(targetTypeName);
                        (0, federation_internals_1.assert)(targetType, 'Context references valid type in the schema');
                        try {
                            ...
                            if (!authRequirementOnContext.satisfies(requirementsOnContextType)) {
                                ...
                                throw federation_internals_1.ERRORS.MISSING_TRANSITIVE_AUTH_REQUIREMENTS.err(msg);
                            }
                            const contextSelectionSet = (0, federation_internals_1.parseSelectionSet)({ parentType: targetType, source: contextSelection });
    ...
    authRequirementsOnElement(element) {
        var _a, _b, _c;
        const requirements = new AuthRequirementsOnElement();
        if (this.authenticatedDirective) {
            const appliedDirective = (_a = element.appliedDirectivesOf(this.authenticatedDirective)) === null || _a === void 0 ? void 0 : _a[0];
            if (appliedDirective) {
                requirements.isAuthenticated = true;
            }
        }
        if (this.requiresScopesDirective) {
            const appliedDirective = (_b = element.appliedDirectivesOf(this.requiresScopesDirective)) === null || _b === void 0 ? void 0 : _b[0];
            if (appliedDirective) {
        ...
        if (this.policyDirective) {
            const appliedDirective = (_c = element.appliedDirectivesOf(this.policyDirective)) === null || _c === void 0 ? void 0 : _c[0];
            if (appliedDirective) {
    ...
    verifyAuthRequirementsOnSelectionSet(authRequirements, selectionSet) {
        for (const selection of selectionSet.selections()) {
            if (selection instanceof federation_internals_1.FieldSelection) {
    ...
    verifyAuthOnFieldSelection(selection, authRequirements) {
        const field = selection.element.definition;
        const fieldAuthReqs = this.authRequirementsOnElement(field);
        const returnType = (0, federation_internals_1.baseType)(field.type);
...
class AuthRequirements {
    constructor(coordinate, directive, typeRequirements, fieldRequirements) {
        var _a, _b;
        this.fieldCoordinate = coordinate;
        this.directive = directive;
        const requirements = new AuthRequirementsOnElement();
        requirements.isAuthenticated = ((_a = typeRequirements === null || typeRequirements === void 0 ? void 0 : typeRequirements.isAuthenticated) !== null && _a !== void 0 ? _a : false) || ((_b = fieldRequirements === null || fieldRequirements === void 0 ? void 0 : fieldRequirements.isAuthenticated) !== null && _b !== void 0 ? _b : false);
        const scopesToMerge = [];
        if (typeRequirements === null || typeRequirements === void 0 ? void 0 : typeRequirements.scopes) {
            scopesToMerge.push(typeRequirements.scopes);
        }
        if (fieldRequirements === null || fieldRequirements === void 0 ? void 0 : fieldRequirements.scopes) {
            scopesToMerge.push(fieldRequirements.scopes);
        }
        if (scopesToMerge.length > 0) {
            requirements.scopes = (0, federation_internals_1.dnfConjunction)(scopesToMerge);
        }
        const policiesToMerge = [];
        if (typeRequirements === null || typeRequirements === void 0 ? void 0 : typeRequirements.policies) {
            policiesToMerge.push(typeRequirements.policies);
        }
        if (fieldRequirements === null || fieldRequirements === void 0 ? void 0 : fieldRequirements.policies) {
            policiesToMerge.push(fieldRequirements.policies);
        }
        if (policiesToMerge.length > 0) {
            requirements.policies = (0, federation_internals_1.dnfConjunction)(policiesToMerge);
        }
        this.requirements = requirements;
    }
    satisfies(authOnElement) {
        if (authOnElement) {
...
class AuthRequirementsOnElement {
    ...
    isImplication(first, second) {
        const firstNormalized = (0, federation_internals_1.convertEmptyToTrue)(first !== null && first !== void 0 ? first : [[]]);
        const secondNormalized = (0, federation_internals_1.convertEmptyToTrue)(second !== null && second !== void 0 ? second : [[]]);
